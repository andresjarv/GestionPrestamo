    ...
    public String Buscar(LinkedList<Estudiante> lista, LinkedList<Dispositivo> lista2, String serialBuscar) {
        ...
        for (Dispositivo disp : lista2) {
            ...
            for (Estudiante est : lista) {
                EstIngenieria e2 = (EstIngenieria) est;
                do {
                    if (lap.getSerial().equalsIgnoreCase(serialBuscar) && e2.getSerial().equalsIgnoreCase(serialBuscar)) {
                    ...
                    } else if (disp.getSerial().equalsIgnoreCase(serialBuscar)
                            && !est.getSerial().equalsIgnoreCase(serialBuscar)) {
                        mensaje = "t El equipo que se busco es \t\n" +
                                disp.MostrarMaestroD() +
                                "Sistema operativo: " + lap.getSistemaOperativo() + "\n" +
                                "Procesador: " + lap.getProcesador() + "\n" +
                ...
                } while (serialBuscar == lap.getSerial());
    ...
    public LinkedList<Estudiante> Modificar(String buscarSerial, LinkedList<Dispositivo> lista,
        ...
        for (Estudiante est : lista2) {
            ...
            for (Dispositivo disp : lista) {
                 Laptop lap = (Laptop) disp;
                do {
                    if (lap.getSerial().equalsIgnoreCase(buscarSerial) && e2.getSerial().equalsIgnoreCase(buscarSerial)) {
                ...
                } while (buscarSerial == lap.getSerial());
            }
    ...
    public LinkedList<Estudiante> Eliminar(String buscarSerial, LinkedList<Estudiante> lista2) {
        ...
        while (flag) {
            buscarSerial = JOptionPane
                    .showInputDialog("Ingrese el serial del dispositico, para la devolucion del equipo: ");
        ...
        for (Estudiante est : lista2) {
            ...
            if (e2.getSerial().equalsIgnoreCase(buscarSerial)) {
                lista2.remove(e2);
                JOptionPane.showMessageDialog(null,
                        "se ha devuelto el equipo correctamente, por ende el estudiante ya no esta vinculado con el dispositivo. Muchas Gracias");
            } else {
                JOptionPane.showMessageDialog(null,
                        "No se ha encontrado un dispositivo con ese serial, intente de nuevo.");
            }
